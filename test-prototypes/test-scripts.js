...
async function runTests() {
  ...
  try {
    const { generateHalftoneLines } = await import('../scripts/brand-assets/pattern-generation/halftone-lines.js');
  ...
  try {
    ...
    const result = await generateSocialMediaTemplate(
      'instagram',
  ...
  try {
    const colors = JSON.parse(readFileSync(join(CONFIG_DIR, 'colors.json'), 'utf8'));
    const result = await generateWebBanner(
      'hero',
      join(TEST_DIR, 'templates'),
      {
        colors,
