...
import { generateHalftoneDots } from "../scripts/pattern-generation/halftone-dots.js"
import { generateHalftoneLines } from "../scripts/pattern-generation/halftone-lines.js"
import { applyDuotone } from "../scripts/image-processing/apply-duotone.js"
import { applyBayerDitherPipeline } from "../scripts/image-processing/apply-bayer-dither.js"
import { batchProcess } from "../scripts/image-processing/batch-process.js"
...
const CONFIG_DIR = join(process.cwd(), "visual-assets", "config")
...
async function generatePrototypes() {
  ...
  // Prototype 0: Bayer Dither - Reference Image Test
  console.log("Prototype 0: Bayer Dither - Reference Image Test")
  const referenceImage = "Carles Rabada Unsplash (1)..jpg"
  const referencePath = join(INPUT_DIR, referenceImage)

  // Test with 4x4 matrix (fine detail)
  await applyBayerDitherPipeline(
    referencePath,
    join(OUTPUT_DIR, "bayer-dither-reference-4x4.png"),
    {
      matrixSize: 4,
      darkColor: colors.bayerDither.darkColor,
      midColor: colors.bayerDither.midColor,
      lightColor: colors.bayerDither.lightColor,
      darkThreshold: colors.bayerDither.darkThreshold,
      lightThreshold: colors.bayerDither.lightThreshold,
    },
  )
  console.log("  ✓ Generated 4x4 matrix variant\n")

  // Test with 8x8 matrix (coarser)
  await applyBayerDitherPipeline(
    referencePath,
    join(OUTPUT_DIR, "bayer-dither-reference-8x8.png"),
    {
      matrixSize: 8,
      darkColor: colors.bayerDither.darkColor,
      midColor: colors.bayerDither.midColor,
  ...
  console.log("  ✓ Generated 8x8 matrix variant\n")

  // Test with different thresholds
  await applyBayerDitherPipeline(
    referencePath,
    join(OUTPUT_DIR, "bayer-dither-reference-custom-thresholds.png"),
    {
      matrixSize: 4,
      darkColor: colors.bayerDither.darkColor,
      midColor: colors.bayerDither.midColor,
  ...
  console.log("  ✓ Generated custom thresholds variant\n")
