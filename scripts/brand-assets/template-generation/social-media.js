...
async function generateSocialMediaTemplate(platform, outputDir, config = {}) {
  try {
    ...
    const colors =
      config.colors ||
      JSON.parse(readFileSync(join(__dirname, "../../../brand-assets/colors.json"), "utf8"))
    ...
    if (config.logoPath) {
      ...
      const scaledLogo = await logo.resize(logoSize, logoSize, { fit: "contain" }).toBuffer()
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  import("fs/promises")
    .then((fs) => fs.mkdir(outputDir, { recursive: true }))
